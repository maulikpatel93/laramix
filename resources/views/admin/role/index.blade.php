...
if (!empty(checkaccess('create', getControllerName()))) {
    $createbtn = '<a href="' . route('admin.role.create') . '" class="btn btn-primary showModalButton" title="Add ' . $title_single . '" data-bs-toggle="modal" data-bs-target="#gridviewModal">Add</a>';
}
...
$actionTypes = [];
if (!empty(checkaccess('access', getControllerName()))) {
    $actionTypes[] = [
        'class' => Itstructure\GridView\Actions\Custom::class, // Required
        'url' => function ($model) {
            // Optional
            return route('admin.role.access', ['id' => encode($model->id)]);
        },
        'htmlAttributes' => [
            // Optional
...
if (!empty(checkaccess('view', getControllerName()))) {
    $actionTypes[] = [
        ...
        'url' => function ($model) {
            // Optional
            return route('admin.role.view', ['id' => encode($model->id)]);
        },
        'htmlAttributes' => [
            // Optional
...
if (!empty(checkaccess('update', getControllerName()))) {
    $actionTypes[] = [
        ...
        'url' => function ($model) {
            // Optional
            return route('admin.role.edit', ['id' => encode($model->id)]);
        },
        'htmlAttributes' => [
            // Optional
...
if (!empty(checkaccess('delete', getControllerName()))) {
    $actionTypes[] = [
        ...
        'url' => function ($model) {
            // Optional
            return route('admin.role.delete', ['id' => encode($model->id)]);
        },
        'htmlAttributes' => [
            // Optional
...
@section('content')
    ...
    $gridData = [
        ...
        'rowsFormAction' => route('admin.role.applystatus'), // Route url to send slected checkbox items for deleting rows, for example.
                ...
                'value' => function ($model) {
                    ...
                    if (!empty(checkaccess('isactive', getControllerName()))) {
                        return Html::link('javascript:void(0)', $active, [
                            ...
                            'onclick' =>
                                '$.post({
                                url: "' .route('admin.role.isactive', ['id' => encode($model->id)]) .'",
        ...
        'toolbar' => [
            'content' =>
                '<div class="btn-group" role="group">
                    ' .$createbtn .'
                    <a href="' .route('admin.role.index') .'" class="btn btn-secondary" title="Refresh Role" data-trigger-pjax="1" ><i class="fas fa-redo"></i></a>
                </div>',
            'applybtn' => $applyafter,
        ],
    ];
